...
def run_glue_spark():  # pragma: no cover
    ...
    spark_transforms.write_iceberg_table(silver_df, database, silver_table)

    silver_df.write.mode("overwrite").parquet(silver_path)
    print(f"[silver] parquet export for quality gate -> {silver_path}")

    q_count = quarantine_df.count()
    if q_count:
        quarantine_path = silver_path.replace("/silver/", "/quarantine/").rstrip("/")
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--bronze",
        default="output/advisory_transactions/bronze/bronze_advisory_transactions.parquet",
    )
