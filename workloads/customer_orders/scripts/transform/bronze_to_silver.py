# spec_hash: 1088d5894d3deae109ff805b0c6bd4901068ed8bff381aab06310d45ccc2c10c
...
"""Bronze -> Silver transform for `customer_orders`.
...
try:
    from workloads.customer_orders.scripts.transform import local_runner, spark_transforms
except ImportError:
...
def run_local(bronze_parquet: str, out_dir: str) -> dict:
    ...
    silver.to_parquet(out / "silver" / "silver_customer_orders.parquet", index=False)
...
def run_glue():  # pragma: no cover - legacy Python Shell path if invoked without Spark
    ...
    silver, quarantine = local_runner.bronze_to_silver(bronze, cfg)
    silver_target = silver_path.rstrip("/") + "/silver_customer_orders.parquet"
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--bronze",
        default="output/customer_orders/bronze/bronze_customer_orders.parquet",
    )
