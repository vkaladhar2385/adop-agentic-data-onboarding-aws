# spec_hash: 648e102fb061173bc75ca093d5c50d3b8e989adea26fbb2d8b785a08b54c7625
...
"""Bronze -> Silver transform for `supplier_lead_times`.
...
try:
    from workloads.supplier_lead_times.scripts.transform import local_runner, spark_transforms
except ImportError:
...
def run_local(bronze_parquet: str, out_dir: str) -> dict:
    ...
    silver.to_parquet(out / "silver" / "silver_supplier_lead_times.parquet", index=False)
...
def run_glue():  # pragma: no cover - legacy Python Shell path if invoked without Spark
    ...
    silver, quarantine = local_runner.bronze_to_silver(bronze, cfg)
    silver_target = silver_path.rstrip("/") + "/silver_supplier_lead_times.parquet"
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--bronze",
        default="output/supplier_lead_times/bronze/bronze_supplier_lead_times.parquet",
    )
