...
"""PySpark helpers for supplier_lead_times Glue ETL jobs."""
...
def bronze_to_silver_df(bronze: DataFrame, cfg: dict | None = None) -> tuple[DataFrame, DataFrame]:
    ...
    sid_blank = F.col("supplier_id").isNull() | (F.trim(F.col("supplier_id")) == "")
    keyed = (
        bronze.filter(~sid_blank)
        .withColumn("_rn", F.row_number().over(window))
        .filter(F.col("_rn") == 1)
        .drop("_rn")
    )
    df = keyed.unionByName(bronze.filter(sid_blank), allowMissingColumns=True)
    ...
    lead = F.col("lead_time_days").cast("double")
    df = (
        df.withColumn("missing_supplier_id", sid_blank)
        .withColumn("invalid_lead_time", lead.isNull() | (lead < 0) | (lead > 365))
    )
    bad = F.col("missing_supplier_id") | F.col("invalid_lead_time")
    quarantine = df.filter(bad).withColumn(
        "quarantine_reason",
        F.concat_ws(
            ",",
            F.when(F.col("missing_supplier_id"), F.lit("missing_supplier_id")),
            F.when(F.col("invalid_lead_time"), F.lit("invalid_lead_time")),
        ),
    )
    silver = (
        df.filter(~bad)
        .drop("missing_supplier_id", "invalid_lead_time")
        .withColumn("lead_time_days", F.col("lead_time_days").cast("int"))
        .withColumn("min_order_qty", F.col("min_order_qty").cast("int"))
        .withColumn("updated_at", F.to_timestamp(F.col("updated_at")))
        .withColumn("effective_date", F.to_date(F.col("effective_date")))
        .withColumn("is_preferred", F.lower(F.col("is_preferred")).isin("true", "1"))
        .withColumn("lead_time_weeks", lead / F.lit(7.0))
    )
...
def silver_to_gold_dfs(silver: DataFrame, cfg: dict | None = None) -> dict[str, DataFrame]:
    ...
    return {s2g.get("table", "gold_supplier_lead_times"): gold}
