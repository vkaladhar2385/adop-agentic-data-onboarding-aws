# Terraform Apply Guide — advisory_transactions + web_events

> Generated by the DevOps Agent. Apply in a **sandbox** account only during the
> pilot. Tear down the same day (see the pilot plan's cost guardrails).
...
└── modules/{redshift,opensearch,redis}_workload/  # extension (advisory_transactions only, optional)
    # each provisions one AWS service + one integration Lambda -- not required
    # for the core pilot demo. See docs/EXTENDING_TO_NEW_SERVICES.md.
```
...
terraform destroy      # leave nothing running overnight
```
